...
def profit_loss(y_true, y_pred):
    ...
    outcomes = tf.reshape(y_true, [tf.size(y_true) / M, M])
    # y_pred is the learned reports
    reports = tf.reshape(
        y_pred, [tf.size(y_pred) / (N_COALITION * M), N_COALITION * M])
    result = tf.map_fn(
        lambda x: calculate_loss_in_profit(x[0], x[1]), (outcomes, reports), dtype=tf.float32)
...
def desire_borrowers() -> None:
    ...
    # y is ignored
    y = np.zeros((N_TEST_CASES, N_COALITION * M)).astype(np.float32)
